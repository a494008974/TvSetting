...
//        	if(action.equals(UsbManager.ACTION_USB_STATE) && isBootCompleted) {
...
//            }
//        }else 
        	if(action.equals(ConnectivityManager.CONNECTIVITY_ACTION) && isBootCompleted) {
        ...
        }else if(action.equals("android.intent.action.CHECK_UPDATE_BY_HAND") && isBootCompleted) {
        			serviceIntent = new Intent(context, RKUpdateService.class);
	                serviceIntent.putExtra("command", RKUpdateService.COMMAND_CHECK_REMOTE_UPDATING_BY_HAND);
					serviceIntent.putExtra("delay", 0);
	                context.startService(serviceIntent);
        }else if(action.equals("p03.2.found.a.otapackage") && isBootCompleted) {
 				Log.d(TAG, "RKUpdateReceiver recv p03-2.found.a.otapackage");
        }
